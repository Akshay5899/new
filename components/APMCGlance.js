      ...
      .then((data) => {
        setNewsData(
          data.slice(0, 5).map((item, index) => ({
            id: item.id,
            date: "Sun, " + (index + 1) + " Jun 2025", // Fake dates
        ...
        {stats.map((item, idx) => (
          <div key={idx} className="col-6 col-sm-4 col-md-2 mb-4">
            <div className="d-flex flex-column align-items-center">
              <img src={item.icon} alt={item.label} style={{ width: '100%', marginBottom: '10px' }} />
...
<div className="card p-4">
  <Tabs defaultActiveKey="timetable" id="apmc-tabs" className="mb-3 custom-tabs">
...
<Tab eventKey="tenders" title="Tenders">
  <div className="mt-3">
    {tenders.map((item) => (
      <div key={item.id} className="text-start border rounded p-3 mb-3" style={{ borderColor: "#0a5c2a" }}>
        <div className="d-flex align-items-center mb-2 text-muted">
          <i className="bi bi-calendar-event me-2"></i>
          {item.date}
        </div>
        ...
        <div>
          <a href={item.link} className="text-success fw-bold me-3">View</a>
          <a href={item.download} className="text-success fw-bold">Download</a>
        </div>
      </div>
    ))}
  </div>
...
<Tab eventKey="circulars" title="Circulars">
  <div className="mt-3">
    {circulars.map((item) => (
      <div key={item.id} className="d-flex flex-column flex-md-row justify-content-between align-items-center border rounded p-3 mb-3" style={{ borderColor: '#0a5c2a' }}>
        <div>
          <h5 className="mb-1 text-success fw-bold">{item.title}</h5>
          <p className="mb-0 text-muted">{item.subtitle}</p>
        </div>
        <div>
          <a href={item.view} className="text-success fw-bold me-3">View</a>
          <a href={item.download} className="text-success fw-bold">Download</a>
        </div>
      </div>
    ))}
